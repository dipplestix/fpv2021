...
| list.nil          a := list.cons a []
| (list.cons x xs)  a := list.cons x (snoc xs a)
...
lemma map_identity {α β : Type}(f : α→β)(x : list α) :
     map ident x = x 
sorry

lemma map_composition {α: Type}(f g : α→α)(x : list α) :
...
--------------------------------------------- LAM
f:α→β, g:γ→α⊢(λ(c:γ)), f(g c)) :β
--------------------------------------------- LAM
f:α→β⊢(λ(g:(γ→α))(c:γ), f(g c)) :β
--------------------------------------------- LAM
⊢(λ(f:(α→β))(g:(γ→α))(c:γ), f(g c)) :β
